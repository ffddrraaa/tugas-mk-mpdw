...
## Impor Data (Data jumlah wisatawan mancanegara di bali yang digunakan yaitu mulai Bulan Januari 2009 hingga Bulan Maret 2020)
...
```{r}
training<-data.turis[1:106,2]
testing<-data.turis[107:133,2]
...
Hasil optimal untuk parameter double exponential smoothing (DES) berdasarkan metrik evaluasi model adalah alpha = 0.7 dan beta = 0.1, sehingga kita akan menggunakan parameter ini untuk melanjutkan metode pemulusan DES.

```{r}
df_des1 <- HoltWinters(training.ts, alpha = 0.7, beta=0.1, gamma=F)
head(df_des1$fitted)
```
...
```{r}
# Hasil Peramalan
ramal_des1 <- forecast::forecast(df_des1,h=27) #periode ramalan sebanyak 27
...
```{r}
plot(ramal_des1, xlab="Tahun", ylab="Jumlah Wisatawan",main="Double Exponential Smoothing (Alpha=0.7, Beta=0.1)")
```
...
title("Double Exponential Smoothing (Alpha=0.7, Beta=0.1)",cex.main=1, font.main=4, col.main="black")
box(col="black",lwd=2)
legend("topleft", c("Data aktual", "Fitted DES","Ramalan"), lty=1,col=c ("blue","#66FF66","#FF3300"))
```

Berdasarkan eksplorasi di atas, plot data training hasil pemulusan menggunakan metode pemulusan DES dengan α = 0.7 dan β = 0.1 memiliki pola hampir sama dengan plot data aktual serta dibandingkan pula dengan data peramalannya.
...
Diperoleh nilai parameter yang optimal yaitu n=2
...
```{r}
 mulussma <- TTR::SMA(training.ts, n=2)
  ...
  mulusdma <- At+Bt
  ramal_dma <- c(NA, mulusdma) 
  df_dma <- cbind(Aktual=training.ts, pemulusan = c(mulusdma,rep(NA,27)), ramal_dma= c(ramal_dma, rep(NA,27)))
  df_dma
```
...
Berdasarkan data testing, model Winter's Aditif merupakan model terbaik dilihat dari perbandingan ukuran kebaikan model di atas.
